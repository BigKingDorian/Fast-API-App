...
async def twilio_voice_webhook(request: Request):
    ...
    if audio_path:
        ...
        logger.info(f"🟥 [User Input] Processing complete — unblocking writes for {call_id}")
        session_memory[call_id]['user_response_processing'] = False
                    ...
                    try:
                        ...
                        if is_final and sentence.strip() and confidence >= 0.6:
                            ...
                            if speech_final:
                                ...
                                session_memory[call_id]['user_response_processing'] = True
                                logger.info(f"🟩 [User Input] Processing started — blocking writes for {call_id}")
